# 配置IP黑白名单规则拦截/放行指定IP<a name="waf_01_0012"></a>
...
-   云模式各版本、独享模式支持创建的IP黑白名单规则条数请参见[服务版本差异](https://support.huaweicloud.com/productdesc-waf/waf_01_0106.html#section1)。
...
2.  单击管理控制台左上角的![](figures/icon-region-33.jpg)，选择区域或项目。
3.  单击页面左上方的![](figures/icon-Service-34.png)，选择“安全与合规  \>  Web应用防火墙 WAF“。
4.  在左侧导航树中，选择“防护策略“，进入“防护策略“页面。
5.  单击目标策略名称，进入目标策略的防护配置页面。
6.  在“黑白名单设置“配置框中，用户可根据自己的需要更改“状态“，单击“自定义黑白名单设置规则“，进入黑白名单设置规则页面。

    **图 1**  黑白名单配置框<a name="fig0358162863015"></a>  
    ![](figures/黑白名单配置框.png "黑白名单配置框")

7.  在“黑白名单设置“页面左上角，单击“添加规则“。
8.  在弹出的对话框中，添加黑白名单规则，如[图2](#fig1810352211544)和[图3](#fig1936217171553)所示，参数说明如[表1](#table147241231818)所示。

    >![](public_sys-resources/icon-note.gif) **说明：** 
    >-   将IP配置为仅记录后，来自该IP的访问，WAF将根据防护规则进行检测并记录该IP的防护事件数据。
    >-   其他的IP将根据配置的WAF防护规则进行检测。

    **图 2**  添加单个IP/IP地址段黑白名单规则<a name="fig1810352211544"></a>  
    ![](figures/添加单个IP-IP地址段黑白名单规则.png "添加单个IP-IP地址段黑白名单规则")

    **图 3**  批量添加IP/IP地址段黑白名单规则<a name="fig1936217171553"></a>  
...
1.  添加以下2条黑白名单规则，拦截所有来源IP。
    ...
    **图 4**  拦截1.0.0.0/1 IP地址段<a name="fig134723543536"></a>  
    ![](figures/拦截1-0-0-0-1-IP地址段.png "拦截1-0-0-0-1-IP地址段")

    **图 5**  拦截128.0.0.0/1 IP地址段<a name="fig13996155195418"></a>  
    ![](figures/拦截128-0-0-0-1-IP地址段.png "拦截128-0-0-0-1-IP地址段")

    您也可以通过添加一条精准访问防护规则，拦截所有访问请求，如[图6](#fig489116305597)所示。

    **图 6**  拦截所有访问请求<a name="fig489116305597"></a>  
    ![](figures/拦截所有访问请求.png "拦截所有访问请求")

    有关配置精准访问防护规则的详细介绍，请参见[配置精准访问防护规则定制化防护策略](配置精准访问防护规则定制化防护策略.md)。

2.  <a name="li839632265215"></a>参照[图7](#fig5519155016115)示例添加黑白名单规则，放行指定IP，例如，XXX.XXX.2.3。

    **图 7**  放行指定IP<a name="fig5519155016115"></a>  
    ![](figures/放行指定IP-35.png "放行指定IP-35")

3.  开启黑白名单防护规则。

    **图 8**  黑白名单配置框<a name="fig17586343102616"></a>  
    ![](figures/黑白名单配置框.png "黑白名单配置框")

4.  清理浏览器缓存，在浏览器中访问“http://www.example.com“页面。

    当访问者的源IP不属于[2](#li839632265215)中设置的放行IP地址时，WAF将拦截该访问请求，拦截页面示例如[图9](#fig11778435913)所示。
