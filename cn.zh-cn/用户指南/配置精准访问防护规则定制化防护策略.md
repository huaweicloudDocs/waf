...
2.  单击管理控制台左上角的![](figures/icon-region-31.jpg)，选择区域或项目。
3.  单击页面左上方的![](figures/icon-Service-32.png)，选择“安全与合规  \>  Web应用防火墙 WAF“。
4.  在左侧导航树中，选择“防护策略“，进入“防护策略“页面。
5.  单击目标策略名称，进入目标策略的防护配置页面。
6.  在“精准访问防护“配置框中，用户可根据自己的需要更改“状态“，单击“自定义精准访问防护规则“，进入精准访问防护规则配置页面。

    **图 1**  精准访问防护配置框<a name="fig275911394277"></a>  
...
7.  在“精准访问防护配置“页面，设置“检测模式“。
    ...
    -   全检测：当用户的请求符合精准防护中的拦截条件时，不会立即拦截，它会继续执行其他防护的检测，待其他防护的检测完成后进行拦截。
        ...
        **图 2**  检测模式<a name="fig1818193165213"></a>  
        ![](figures/检测模式.png "检测模式")

8.  在“精准访问防护配置“页面左上角，单击“添加规则“。
9.  在弹出的对话框中，根据[表1](#table2299936310457)添加精准访问防护规则。

    以[图3](#fig39459217174738)的配置为例，其含义为：当用户访问目标域名下包含“/admin“的URL地址时，WAF将阻断该用户访问目标URL地址。
...
假如已添加域名“www.example.com“，且配置了如[图3](#fig39459217174738)所示的精准访问防护规则。可参照以下步骤验证防护效果：
...
通过分析某类特定的WordPress反弹攻击，发现其特征是User-Agent字段都包含WordPress，如[图4](#fig16451834185616)所示。

**图 4**  WordPress反弹攻击<a name="fig16451834185616"></a>  
![](figures/WordPress反弹攻击.png "WordPress反弹攻击")

因此，可以设置精准访问控制规则，拦截该类WordPress反弹攻击请求。

**图 5** **User Agent配置**<a name="fig116717261909"></a>  
![](figures/User-Agent配置.png "User-Agent配置")

## 配置示例-拦截特定的URL请求<a name="section199707217912"></a>

如果您遇到有大量IP在访问某个特定且不存在的URL，您可以通过配置以下精准访问防护规则直接阻断所有该类请求，降低源站服务器的资源消耗，如[图6](#fig169721525916)所示。

**图 6**  特定的URL拦截<a name="fig169721525916"></a>  
![](figures/特定的URL拦截.png "特定的URL拦截")

## 配置示例-拦截字段为空值的请求<a name="section12135201518352"></a>

如果您需要拦截某个为空值的字段，您可以通过配置精准访问防护规则直接阻断该类请求，如[图7](#fig6135111553516)所示。

**图 7**  Referer空值拦截<a name="fig6135111553516"></a>  
![](figures/Referer空值拦截.png "Referer空值拦截")

## 配置示例-拦截指定文件类型（zip、tar、docx等）<a name="section699886111110"></a>

通过配置路径字段匹配的文件类型，您可以阻断特定的文件类型。例如，您需要拦截“.zip“格式文件，您可以配置精准防护规则阻断“.zip“文件类型访问请求，如[图8](#fig1599818616112)所示。

**图 8**  阻断特定文件类型请求<a name="fig1599818616112"></a>  
...
**图 9**  防盗链<a name="fig1330993917144"></a>  
![](figures/防盗链.png "防盗链")

## 配置示例-单独放行指定IP的访问<a name="section56488214156"></a>

配置两条精准访问防护规则，一条拦截所有的请求，如[图10](#fig11661145013158)所示，一条单独放行指定IP的访问，如[图11](#fig866195019151)所示。

**图 10**  阻断所有的请求<a name="fig11661145013158"></a>  
![](figures/阻断所有的请求.png "阻断所有的请求")

**图 11**  放行指定IP<a name="fig866195019151"></a>  
![](figures/放行指定IP.png "放行指定IP")

## 配置示例-放行指定IP的特定URL请求<a name="section11765134112520"></a>

通过配置多条“条件列表“，当访问请求同时满足条件列表时，可以实现放行指定IP的特定URL请求，如[图12](#fig1976618419256)所示。

**图 12**  放行指定IP访问特定路径<a name="fig1976618419256"></a>  
