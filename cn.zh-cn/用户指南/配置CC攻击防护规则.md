...
CC攻击是攻击者通过控制代理服务器向服务器发送大量貌似合法的请求，直至服务器资源耗尽、宕机崩溃。

CC攻击防护规则支持通过限制单个IP/Cookie/Referer访问者对防护网站上特定路径（URL）的访问频率，精准识别CC攻击以及有效缓解CC攻击。例如，通过配置CC攻击防护规则实现以下功能：当Cookie标识为name的用户在60秒内访问您域名下的“/admin\*“页面超过10次时，封禁该用户访问目标网址600秒。

CC攻击防护规则支持“人机验证“、“阻断“、“动态阻断“和“仅记录“（只记录不阻断）防护动作，当您配置完CC攻击防护规则并开启CC攻击防护后（即“CC攻击防护“配置框的“状态“为![](figures/icon-enable.png)），WAF将根据您配置的CC攻击防护规则进行CC攻击防护。
