...
5.  单击“确认添加“，添加该误报规则。
    ...
    >![](public_sys-resources/icon-note.gif) **说明：** 
    >-   设置误报屏蔽后，1分钟左右生效，攻击事件详情列表中将不再出现此误报。您可以刷新浏览器缓存，重新访问设置了误报屏蔽的页面，验证是否配置成功。
    >-   处理误报后，您可以在误报屏蔽规则列表中，查看该误报事件对应的误报屏蔽规则。如果需要继续拦截该攻击事件，您可以关闭或删除添加的误报屏蔽规则。有关配置误报屏蔽规则的详细操作，请参见[配置误报屏蔽规则](zh-cn_topic_0110861348.md)。
    >-   如果您已经对某个攻击事件进行了误报处理，在该攻击事件所在行的“操作“列中，单击“误报处理“时，将不能重复添加误报屏蔽规则。
