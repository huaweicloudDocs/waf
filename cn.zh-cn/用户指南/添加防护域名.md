...
该任务指导您将网站域名添加到Web应用防火墙，并完成域名接入，使网站流量切入WAF。域名接入WAF后，WAF作为一个反向代理存在于客户端和服务器之间，服务器的真实IP被隐藏起来，Web访问者只能看到WAF的IP地址。
...
6.  在“添加防护域名“页面配置域名基本信息，如[图4](#fig175731754141418)所示，相关参数说明如[表1](#table7692122554811)所示。
    ...
    **表 1**  基本信息参数说明
    ...
    <p id="p12688525104820"><a name="p12688525104820"></a><a name="p12688525104820"></a>Web应用防火墙可支持非标准端口173个（HTTP业务端口148个，HTTPS业务端口25个）。</p>
    </td>
...
9.  单击“下一步“，进入“域名接入“页面，同时在页面的右上角，会弹出“域名添加成功“。
    ...
    -   未使用代理，需要配置CNAME记录。
        ...
        2.  验证域名的CNAME是否配置成功。
            ...
            2.  执行以下命令，查询CNAME。如果回显的域名是配置的CNAME，则表示配置成功，示例如[图6](#zh-cn_topic_0110861266_waf_01_0079_fig04371756152219)所示。

                **nslookup www.**_domain_**.com**

                **图 6**  查询CNAME<a name="zh-cn_topic_0110861266_waf_01_0079_fig04371756152219"></a>  
...
11. 单击“完成“，防护域名添加成功。
    ...
    >-   若防护域名已接入WAF，“DNS解析状态“仍然为“异常“，可单击![](figures/刷新图标.jpg)，刷新状态，如果仍然为“异常“，可参照[域名接入](域名接入.md)重新进行域名接入。  
...
-   使用WAF做HTTPS卸载模式，如[图10](#fig11273129104514)所示。

    **图 10**  使用WAF做HTTPS卸载模式<a name="fig11273129104514"></a>  
