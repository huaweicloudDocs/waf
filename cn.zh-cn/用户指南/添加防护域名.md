...
## 操作须知<a name="section8698135116389"></a>
...
4.  选择“云模式“后，在“域名配置“页面配置域名基本信息，如[图4](#fig175731754141418)所示，相关参数说明如[表1](#table7692122554811)所示。
    ...
    **表 1**  基本信息参数说明
    ...
    <div class="notice" id="note12994653101212"><a name="note12994653101212"></a><a name="note12994653101212"></a><span class="noticetitle"> 须知： </span><div class="noticebody"><p id="p9994853161215"><a name="p9994853161215"></a><a name="p9994853161215"></a>当前仅<span class="parmname" id="parmname139304481307"><a name="parmname139304481307"></a><a name="parmname139304481307"></a>“华北”</span>区域支持IPv6防护，且仅企业版和旗舰版支持IPv6。</p>
    </div></div>
    ...
    <div class="notice" id="note11454155319244"><a name="note11454155319244"></a><a name="note11454155319244"></a><span class="noticetitle"> 须知： </span><div class="noticebody"><a name="waf_01_0262_ul796314498014"></a><a name="waf_01_0262_ul796314498014"></a><ul id="waf_01_0262_ul796314498014"><li>更新证书不会影响业务，更换过程中会使用旧证书，更新成功后，自动切为新证书。</li><li>同时更新后端服务器上的证书配置和WAF域名绑定证书的配置会影响网站访问业务，建议您在业务量少时进行更新。</li><li>WAF不支持证书过期时发送告警通知功能，您可以在<span class="wintitle" id="waf_01_0262_wintitle124641837111118"><a name="waf_01_0262_wintitle124641837111118"></a><a name="waf_01_0262_wintitle124641837111118"></a>“证书管理”</span>界面查看证书的过期时间。</li><li>证书过期后，对源站的影响是覆灭性的，比主机崩溃和网站无法访问的影响还要大，且会造成WAF的防护规则不生效，故建议您在证书到期前及时更新证书。</li><li>域名和证书需要一一对应，泛域名只能使用泛域名证书。如果您没有泛域名证书，只有单域名对应的证书，则只能在WAF中按照单域名的方式逐条添加域名进行防护。</li></ul>
    </div></div>
    </td>
...
7.  单击“下一步“，进入“域名接入“页面，同时在页面的右上角，会弹出“域名添加成功“。

    如果您暂时不接入域名，可跳过本步骤，直接单击“下一步“后单击“完成“。后续再参照[域名接入](域名接入WAF.md)章节完成域名接入。
    ...
    -   如果您使用了CDN或高防等代理类服务，页面如[图6](#fig32845361455)所示。
        ...
        >![](public_sys-resources/icon-note.gif) **说明：** 
        >-   为了确保WAF转发正常，在修改DNS解析配置前，建议您参照[本地验证](本地验证.md)进行本地验证确保一切配置正常。
...
8.  （可选）验证域名的CNAME是否配置成功。
    ...
    2.  执行**nslookup**命令，查询CNAME。
        ...
        如果回显的域名是配置的CNAME，则表示配置成功，示例如[图10](#waf_01_0079_fig1190717481633)所示。
        ...
        **图 10**  查询CNAME<a name="waf_01_0079_fig1190717481633"></a>  
...
9.  单击“下一步“后单击“完成“，防护域名添加成功。
    ...
    >-   若防护域名已接入WAF，“接入状态“仍然为“未接入“，可单击![](figures/icon-refresh.png)，刷新状态，如果仍然为“未接入“，可参照[域名接入WAF](域名接入WAF.md)重新完成域名接入。


## 配置示例一：防护同一端口的不同源站IP的标准端口业务<a name="section04061341181016"></a>

1.  配置时，不勾选非标准端口。
2.  “对外协议“统一选择“HTTP“或者“HTTPS“。HTTP标准端口防护配置如[图11](#fig15777310276)所示，HTTPS标准端口防护配置如[图12](#fig1830919526272)所示。

    **图 11**  80端口业务<a name="fig15777310276"></a>  
    ![](figures/80端口业务.png "80端口业务")

    **图 12**  443端口业务<a name="fig1830919526272"></a>  
...
2.  “对外协议“全部选择“HTTP“或者“HTTPS“。HTTP协议的非标准端口的配置如[图13](#fig12630226124114)，HTTPS协议的非标准端口的配置如[图14](#fig10484211104214)。

    **图 13**  除80端口的其他HTTP协议端口的业务<a name="fig12630226124114"></a>  
    ![](figures/除80端口的其他HTTP协议端口的业务.png "除80端口的其他HTTP协议端口的业务")

    **图 14**  除443端口的其他HTTPS协议端口的业务<a name="fig10484211104214"></a>  
...
如果防护的业务端口不一样，则需要分别添加域名进行配置，如：域名www.example.com需要同时防护8080端口和6443端口，配置如[图15](#fig1492713211584)和[图16](#fig88015455919)所示。

**图 15**  8080端口<a name="fig1492713211584"></a>  
![](figures/8080端口.png "8080端口")

**图 16**  6443端口<a name="fig88015455919"></a>  
...
-   HTTP访问模式，如[图17](#fig53041342142615)所示。

    **图 17**  HTTP协议访问模式<a name="fig53041342142615"></a>  
...
-   HTTPS访问模式，客户端协议全部配置为HTTPS时，当使用HTTP协议访问服务器时，会强制跳转为HTTPS协议，如[图18](#fig7444410153315)所示。

    **图 18**  HTTPS协议访问强制跳转模式<a name="fig7444410153315"></a>  
...
-   HTTP/HTTPS分别转发模式，如[图19](#fig3389134713400)所示。

    **图 19**  HTTP/HTTPS分别转发模式<a name="fig3389134713400"></a>  
...
-   使用WAF做HTTPS卸载模式，如[图20](#fig11273129104514)所示。

    **图 20**  使用WAF做HTTPS卸载模式<a name="fig11273129104514"></a>  
