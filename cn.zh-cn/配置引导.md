...
## WAF引擎检测图<a name="section8980164913235"></a>

Web应用防火墙内置的自定义的防护规则，可帮助您防范常见的Web应用攻击，包括XSS攻击、SQL注入、爬虫检测、Webshell检测等。同时，您也可以根据自己网站防护的需要，灵活配置防护规则，Web应用防火墙根据您配置的防护规则更好的防护您的网站业务。WAF引擎针对自定义规则和内置防护规则的检测流程如[图1](#fig1628214208241)所示。
...
>域名添加到WAF后，WAF会自动为该域名绑定一个空的防护策略，为域名配置的防护规则默认也添加到绑定该域名的防护策略。如果以后有适用于该防护策略的域名，可直接通过该策略添加防护域名，具体的操作请参见[添加策略适用的防护域名](添加策略适用的防护域名.md)。  

-   入口

    **图 2**  防护配置入口<a name="fig443351912108"></a>  
...
-   入口
    ...
    **图 3**  进入防护策略入口<a name="fig4203459164711"></a>  
